...
func GetRootDirId(db database.DatabaseStore) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rootNode, err := db.GetRootNode(googleId)
		if err != nil {
			c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{
...
func GetCurrDirElements(db database.DatabaseStore) gin.HandlerFunc {
	return func(c *gin.Context) {
		googleId, err := utils.GoogleIdstring(c)
		if err != nil {
			c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{
		...
		children, err := db.ListChildren(&parentIdUUID, googleId)
		if err != nil {
			c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{
